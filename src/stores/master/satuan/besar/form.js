...
import { useSatuanBesarStore } from './crud'
import { hurufBesar } from 'src/modules/formatter'
import { Dialog } from 'quasar'
import { useProdukFormStore } from '../../produk/form'

export const useSatuanBesarFormStore = defineStore('satuan_besar_form', {
  state: () => ({
    isOpen: false,
    form: {
      nama: ''
  ...
  actions: {
    ...
    addSatuan(val) {
      Dialog.create({
        title: 'Konfirmasi',
        message: `Apakah <strong>Satuan Besar: ${val}</strong> Akan ditambahkan?`,
        cancel: true,
        html: true
        // persistent: true
      })
    ...
    saveForm() {
      ...
      return new Promise((resolve, reject) => {
        api
          .post('v1/satuan/store-besar', this.form)
          .then((resp) => {
